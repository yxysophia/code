...
class Work
{
    private String name;  //记录作业名字
    private int size;     //记录作业申请存储空间大小
...
class  MenMeth
{
    public void memSort(ArrayList<Memorys> list)
    {
        //使用冒泡排序
        int flag=1;
        for(int i=1;i<list.size();i++)
        {
            for(int j=0;j<list.size()-i;j++)
            {
                ...
                {
                    flag=0;
                    Memorys memorys=list.get(j);  //因为交换需要将整个存储块单元进行交换，包括存储块的大小和状态
    ...
    //打印存储块
    public void printMem(ArrayList<Memorys> list)
    {
...
class Process
{
    ...
    //使用最先适应算法分配及回收内存
    public void mainPro(ArrayList<Work> worksList, ArrayList<Memorys> memorysList) {
        while (true) {

            System.out.println("请输入你要操作的步骤：");
            System.out.println("1.作业申请内存");
            System.out.println("2.结束作业，回收内存");
            int n = input.nextInt();

            //申请内存
            if (n == 1) {
                System.out.println("请输入要申请内存的作业名(作业?)");
                    ...
                    if (workName.equals(worksList.get(i).getName())) {
                        ...
                        for (j = 0; j < memorysList.size(); j++) {
                            ...
                            if (worksList.get(i).getSize() <= memorysList.get(j).getSize()
                                ...
                                memorysList.get(j).setState(workName + "正在运行");
                                if (beyond != 0) {
            ...
            //结束作业，回收内存
                ...
                System.out.println("请输入要结束的作业名称");
                String workName = input.next();
                int i=0;
...
public class Mem {
    ...
    public static void main(String[] args)
    {
        ...
        ArrayList<Memorys> memorysList=new ArrayList();
